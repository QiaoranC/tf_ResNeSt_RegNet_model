...
class ResNest():
    ...
    def _SplAtConv2d(self,input_tensor,filters=64,kernel_size=3, stride=1,
        ...
        x = GroupedConv2D(filters=filters*radix,kernel_size=[kernel_size for i in range(groups*radix)], 
                          ...
                          use_bias=False,data_format='channels_last',dilation_rate=dilation)(x)
        ...
        # print('sum',gap.shape)
        gap = layers.GlobalAveragePooling2D(data_format='channels_last')(gap)
        ...
        x = layers.Conv2D(inter_channels,kernel_size=1)(gap)

        x = BatchNormalization(axis=self.channel_axis,epsilon=1.001e-5)(x)
        x = Activation(self.active)(x)
        x = layers.Conv2D(filters*radix,kernel_size=1)(x)
    ...
    def _make_block(self,input_tensor,first_block=True,filters=64,stride=2,
        ...
        if stride != 1 or inplanes != filters * self.block_expansion:
            ...
            if self.avg_down:
                if self.dilation == 1:
                    short_cut = layers.AveragePooling2D(pool_size=stride, strides=stride,
                                        padding='same',data_format='channels_last')(short_cut)
                else:
                    short_cut = layers.AveragePooling2D(pool_size=1, strides=1,
                                        padding='same',data_format='channels_last')(short_cut)
                short_cut = layers.Conv2D(filters*self.block_expansion,kernel_size=1,strides=1,
                        padding='same',kernel_initializer='he_normal',
                        use_bias=False,data_format='channels_last')(short_cut)
            else:
                short_cut = layers.Conv2D(filters*self.block_expansion,kernel_size=1,strides=stride,
                        padding='same',kernel_initializer='he_normal',
                        use_bias=False,data_format='channels_last')(short_cut)
                        
            short_cut = layers.BatchNormalization(axis=self.channel_axis,epsilon=1.001e-5)(short_cut)
        else:
            short_cut = input_tensor

        group_width = int(filters * (self.bottleneck_width / 64.)) * self.cardinality
        x = layers.Conv2D(group_width,kernel_size=1,strides=1,
                       padding='same',kernel_initializer='he_normal',
                       use_bias=False,data_format='channels_last')(x)
        x = layers.BatchNormalization(axis=self.channel_axis,epsilon=1.001e-5)(x)
        x = layers.Activation(self.active)(x)
        ...
        if avd:
            avd_layer = layers.AveragePooling2D(pool_size=3, strides=stride,
        ...
        if radix >= 1:
            x = self._SplAtConv2d(x,filters=group_width,kernel_size=3, stride=stride,
                                    dilation=self.dilation,groups=self.cardinality,radix=radix)
        else:
            x = layers.Conv2D(group_width,kernel_size=3,strides=stride,
                       padding='same',kernel_initializer='he_normal',
                       dilation_rate=self.dilation,use_bias=False,
        ...
        if avd and not avd_first:
            x = avd_layer(x)
            # print('can')
        x = layers.Conv2D(filters * self.block_expansion,kernel_size=1,strides=1,
                       padding='same',kernel_initializer='he_normal',
                       dilation_rate=self.dilation,use_bias=False,
                       data_format='channels_last')(x)
        x = layers.BatchNormalization(axis=self.channel_axis,epsilon=1.001e-5)(x)
        ...
        m2 = Add()([x, short_cut])
        m2 = layers.Activation(self.active)(m2)
        return m2

    def _make_block_basic(self,input_tensor,first_block=True,filters=64,stride=2,
                      radix=1,avd=False,avd_first=False,is_first=False):
        ...
        if stride != 1 or inplanes != filters * self.block_expansion:
            if self.avg_down:
                if self.dilation == 1:
                    ...
                    short_cut = layers.AveragePooling2D(pool_size=1, strides=1,
                                        padding='same',data_format='channels_last')(short_cut)
                short_cut = layers.Conv2D(filters,kernel_size=1,strides=1,
                        padding='same',kernel_initializer='he_normal',
                        use_bias=False,data_format='channels_last')(short_cut)
            else:
                short_cut = layers.Conv2D(filters,kernel_size=1,strides=stride,
                        padding='same',kernel_initializer='he_normal',
                        use_bias=False,data_format='channels_last')(short_cut)
    ...
    def _make_layer(self,input_tensor,blocks=4,filters=64,stride=2,is_first=True):
        ...
        if self.using_basic_block is True:
            x = self._make_block_basic(x,first_block=True,filters=filters,stride=stride,
                                radix=self.radix,avd=self.avd, avd_first=self.avd_first,
                                is_first=is_first)
            # print('0',x.shape)

            for i in range(1,blocks):
                x = self._make_block_basic(x,first_block=False,filters=filters,stride=1,
                                    radix=self.radix,avd=self.avd, avd_first=self.avd_first)
                # print(i,x.shape)
        
        elif self.using_basic_block is False:
            x = self._make_block(x,first_block=True,filters=filters,stride=stride,
                                radix=self.radix,avd=self.avd, avd_first=self.avd_first,
                                is_first=is_first)
            # print('0',x.shape)
    ...
    def build(self):
        ...
        if self.verbose: print("Resnest builded with input {}, output{}".format(input_sig.shape,fc_out.shape))
...
#         self.verbose=verbose
#     def build(self):
#         model = ResNest(cfg,verbose=self.verbose).build()
...
if __name__ == "__main__":
    ...
    args_config = '../configs/RuiJing_refine82_tfdata_img_resnest50.json'
    cfg = load_cfg(defaut_cfg_path,args_config)
    # print(cfg.__dict__)
    model = ResNest(cfg,verbose=True).build()
    # print('123')
